...
const foodGeo = new THREE.SphereGeometry(0.002, 8, 6)
...
export default function FishFood({ pelletsRef, bounds }: FishFoodProps) {
  ...
  useFrame((state, delta) => {
    ...
    for (let i = pellets.length - 1; i >= 0; i--) {
      ...
      p.velocity.y = Math.max(p.velocity.y - 0.02 * dt, -0.05)
      p.velocity.x *= 1 - 0.5 * dt
      p.velocity.z *= 1 - 0.5 * dt

      p.position.addScaledVector(p.velocity, dt)
      p.position.x += Math.sin(t * 2.5 + p.id * 1.7) * 0.0075 * dt
      p.position.z += Math.cos(t * 2.0 + p.id * 2.3) * 0.0075 * dt

      if (bounds.shape === 'cylinder') {
        const distXZ = Math.sqrt(p.position.x * p.position.x + p.position.z * p.position.z)
        if (distXZ > bounds.radius) {
          const s = bounds.radius / distXZ
          p.position.x *= s
          p.position.z *= s
        }
